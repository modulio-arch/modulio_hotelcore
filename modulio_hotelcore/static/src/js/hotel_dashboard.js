...
class HotelDashboard extends Component {
    setup() {
        ...
        this.state = useState({
            loading: true,
            kpi: { total: 0, available: 0, occupied: 0, reserved: 0, dirty: 0, event: 0, oos: 0 },
            roomTypes: [],
            rooms: [],
    ...
    async loadData() {
        try {
            ...
            // Fetch KPI summary per status via search_count
            ...
            if (this.state) {
                this.state.kpi = { total, available, occupied, reserved, dirty, event, oos };
            }
            ...
            const rooms = await this.orm.searchRead("hotel.room", domain, ["room_number", "floor", "room_type_id", "status", "maintenance_required"], { limit: 40 });
            if (this.state) {
